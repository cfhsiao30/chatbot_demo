...
# 7️⃣ 行程卡片顯示
# ============================================================
def show_spot_card(spot: dict, col):
    with col:
        name = spot['name']
        ...
        with img_col:
            if img:
                ...
                if source == 'wikimedia':
                    st.caption("圖片來源：Wikimedia Commons（CC BY-SA）")
            else:
                st.markdown("""
                <div style="background:#D4CFC7; height:140px; display:flex; align-items:center;
                            justify-content:center; border-radius:8px; color:#6B6B6B;
        ...
        with info_col:
            st.markdown(f"""
            <div style="background:#F5F0E8; padding:14px; border-radius:8px;
                        border-left:4px solid #5B8A5B; height:100%;">
                <p style="margin:4px 0; font-weight:bold; color:#3D5A3D; font-size:15px;">📍 {name}</p>
                <p style="margin:8px 0 4px 0; color:#555; font-size:13px;">⏱️ {spot['duration']}</p>
                <p style="margin:4px 0; color:#555; font-size:13px;">🍽️ {spot['food']}</p>
                <p style="margin:4px 0; color:#E07B39; font-size:13px;">💡 {spot['tip']}</p>
            </div>
            """, unsafe_allow_html=True)
...
def show_itinerary_cards(itinerary_data: dict):
    for day_data in itinerary_data['days']:
        ...
        spot_cols = [cols[i * 2] for i in range(n)]

        for i, spot in enumerate(spots):
            show_spot_card(spot, spot_cols[i])

        for i in range(n - 1):
            with cols[i * 2 + 1]:
                st.markdown("""
                <div style="display:flex; align-items:center; justify-content:center;
                            padding-top:80px; color:#A0845C; font-size:28px;">▶</div>
                """, unsafe_allow_html=True)
...
# ============================================================
# 4️⃣ 照片輔助函式（方案 C1：本地 images/ 資料夾）
# ============================================================
def load_spot_image(name: str):
    """依景點中文名稱尋找 images/ 資料夾內的對應圖片，找不到回傳 None。"""
    for ext in ['.jpg', '.jpeg', '.png', '.webp']:
        path = Path(f"images/{name}{ext}")
        if path.exists():
            return str(path)
...
def show_spot_card(spot: dict, col):
    with col:
        name = spot['name']
        img_path = load_spot_image(name)

        if img_path:
            st.image(img_path, use_container_width=True)
        else:
            st.markdown(f"""
            <div style="background:#D4CFC7; height:120px; display:flex; align-items:center;
                        justify-content:center; border-radius:8px; color:#6B6B6B;
        ...
        <div style="background:#F5F0E8; padding:12px; border-radius:8px;
                    border-left:4px solid #5B8A5B; min-height:110px;">
            <p style="margin:4px 0; font-weight:bold; color:#3D5A3D; font-size:15px;">📍 {name}</p>
            <p style="margin:4px 0; color:#555; font-size:13px;">⏱️ {spot['duration']}</p>
            <p style="margin:4px 0; color:#555; font-size:13px;">🍽️ {spot['food']}</p>
            <p style="margin:4px 0; color:#E07B39; font-size:13px;">💡 {spot['tip']}</p>
        </div>
        """, unsafe_allow_html=True)
...
def show_itinerary_cards(itinerary_data: dict):
    for day_data in itinerary_data['days']:
        ...
        st.markdown(f"""
        <div style="background:#5B8A5B; color:white; padding:10px 20px;
                    border-radius:8px; margin:24px 0 12px 0;">
            <h3 style="margin:0;">📅 第 {day_num} 天行程</h3>
        </div>
        """, unsafe_allow_html=True)
